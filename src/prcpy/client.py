...
class _ClientBase:
    def __init__(self, server_key: str, *, global_api_key: str | None = None, base_url: str = BASE_URL, timeout: float | httpx.Timeout = 30.0) -> None:
    ...
    def from_env(cls, **kwargs: Any):
        ...
        return cls(server_key, global_api_key=os.getenv("ERLC_GLOBAL_API_KEY"), **kwargs)

    def _headers(self) -> dict[str, str]:
        headers = {
            "Accept": "application/json",
            "server-key": self.server_key,
            "User-Agent": "prcpy/0.1.0",
        }
    ...
    def _include_params(flags: dict[str, bool]) -> dict[str, str]:
        ...
        return {_INCLUDE_MAP[name]: "true" for name, enabled in flags.items() if enabled}
    ...
    def _raise_for_error(self, response: httpx.Response) -> None:
        ...
        if isinstance(payload, dict):
            ...
            error_code = None
            command_id = None
        common = {"status_code": response.status_code, "error_code": error_code, "details": payload, "command_id": command_id}
        ...
        if response.status_code == 429:
            ...
            raise RateLimitError(message, retry_after=retry_after_value, bucket=self.rate_limit.bucket if self.rate_limit else None, **common)
    ...
    def _json(response: httpx.Response) -> Any:
        ...
        try:
            ...
            raise APIError("The API returned an invalid JSON response.", status_code=response.status_code, details=response.text) from exc
...
class Client(_ClientBase):
    ...
    def request(self, method: str, path: str, **kwargs: Any) -> Any:
        response = self._http.request(method, f"{self.base_url}{path}" if path.startswith("/") else path, headers=self._headers(), **kwargs)
        self.rate_limit = RateLimitInfo.from_headers(response.headers)
        self._raise_for_error(response)
        return self._json(response)

    def get_server(self, *, players: bool = False, staff: bool = False, join_logs: bool = False, queue: bool = False, kill_logs: bool = False, command_logs: bool = False, mod_calls: bool = False, emergency_calls: bool = False, vehicles: bool = False) -> Server:
        flags = {"players": players, "staff": staff, "join_logs": join_logs, "queue": queue, "kill_logs": kill_logs, "command_logs": command_logs, "mod_calls": mod_calls, "emergency_calls": emergency_calls, "vehicles": vehicles}
    ...
    def send_command(self, command: str) -> CommandResult:
        ...
        return CommandResult.from_api(self.request("POST", "/v2/server/command", json={"command": command}))
...
class AsyncClient(_ClientBase):
    ...
    async def request(self, method: str, path: str, **kwargs: Any) -> Any:
        response = await self._http.request(method, f"{self.base_url}{path}" if path.startswith("/") else path, headers=self._headers(), **kwargs)
        self.rate_limit = RateLimitInfo.from_headers(response.headers)
        self._raise_for_error(response)
        return self._json(response)

    async def get_server(self, *, players: bool = False, staff: bool = False, join_logs: bool = False, queue: bool = False, kill_logs: bool = False, command_logs: bool = False, mod_calls: bool = False, emergency_calls: bool = False, vehicles: bool = False) -> Server:
        flags = {"players": players, "staff": staff, "join_logs": join_logs, "queue": queue, "kill_logs": kill_logs, "command_logs": command_logs, "mod_calls": mod_calls, "emergency_calls": emergency_calls, "vehicles": vehicles}
        return Server.from_api(await self.request("GET", "/v2/server", params=self._include_params(flags)))

    async def get_players(self) -> list[Player]:
        return [Player.from_api(item) for item in await self.request("GET", "/v1/server/players")]
    ...
    async def get_join_logs(self) -> list[JoinLog]:
        return [JoinLog.from_api(item) for item in await self.request("GET", "/v1/server/joinlogs")]

    async def get_queue(self) -> list[int]:
        return await self.request("GET", "/v1/server/queue")

    async def get_kill_logs(self) -> list[KillLog]:
        return [KillLog.from_api(item) for item in await self.request("GET", "/v1/server/killlogs")]

    async def get_command_logs(self) -> list[CommandLog]:
        return [CommandLog.from_api(item) for item in await self.request("GET", "/v1/server/commandlogs")]

    async def get_mod_calls(self) -> list[ModCall]:
        return [ModCall.from_api(item) for item in await self.request("GET", "/v1/server/modcalls")]

    async def get_bans(self) -> dict[str, Any]:
        return await self.request("GET", "/v1/server/bans")

    async def get_vehicles(self) -> list[Vehicle]:
        return [Vehicle.from_api(item) for item in await self.request("GET", "/v1/server/vehicles")]

    async def send_command(self, command: str) -> CommandResult:
        if not command.strip():
            raise ValueError("command must not be empty")
        return CommandResult.from_api(await self.request("POST", "/v2/server/command", json={"command": command}))
