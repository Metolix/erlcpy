...
class Client(_ClientBase):
    ...
    def get_server(
        ...
        *,
        ...
        return Server.from_api(
            self.request("GET", "/v2/server", params=self._include_params(flags))
        )

    def get_players(self) -> list[Player]:
        return [
            Player.from_api(item)
            for item in self.request("GET", "/v1/server/players")
        ]
    ...
    def get_join_logs(self) -> list[JoinLog]:
        return [
            JoinLog.from_api(item)
            for item in self.request("GET", "/v1/server/joinlogs")
        ]
    ...
    def get_kill_logs(self) -> list[KillLog]:
        return [
            KillLog.from_api(item)
            for item in self.request("GET", "/v1/server/killlogs")
        ]

    def get_command_logs(self) -> list[CommandLog]:
        return [
            CommandLog.from_api(item)
            for item in self.request("GET", "/v1/server/commandlogs")
        ]

    def get_mod_calls(self) -> list[ModCall]:
        return [
            ModCall.from_api(item)
            for item in self.request("GET", "/v1/server/modcalls")
        ]
    ...
    def get_vehicles(self) -> list[Vehicle]:
        return [
            ...
            for item in self.request("GET", "/v1/server/vehicles")
        ]
    ...
    async def get_server(
        ...
        *,
        ...
        data = await self.request(
            "GET",
            "/v2/server",
            params=self._include_params(flags),
        )
